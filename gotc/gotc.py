...
def sinkhorn_calibrate(
    ...
    """Calibrate similarity scores using entropic OT (Sinkhorn)."""
...
def knn_graph_from_scores(S_ot: ArrayLike, k: int = 20, sym: bool = True) -> sparse.csr_matrix:
    """Build row-normalized kNN adjacency from score matrix (CSR)."""
    ...
    col = idx.reshape(-1)
    data = np.maximum(sim[np.arange(n)[:, None], idx].reshape(-1), 0.0)
...
def propagate_scores(S_ot: ArrayLike, A: sparse.csr_matrix, steps: int = 2, alpha: float = 0.9) -> ArrayLike:
    """Propagate query-target scores over graph: S_{t+1}=alpha*A*S_t+(1-alpha)*S0."""
...
def _confidence_fusion(S_ot: ArrayLike, S_q: ArrayLike, S_t: ArrayLike) -> ArrayLike:
    ...
    row_max = S_ref.max(axis=1, keepdims=True)
    row_mean = S_ref.mean(axis=1, keepdims=True)
    conf = row_max - row_mean
    conf = (conf - conf.min()) / np.maximum(conf.max() - conf.min(), 1e-12)
    w = 0.2 + 0.5 * conf
    return w * S_avg + (1.0 - w) * S_ref
...
def gotc(
    ...
    """Alternate OT calibration and two-sided relation propagation."""
    S_cur = np.asarray(S, dtype=np.float64)

    for _ in range(max(1, outer_iters)):
        S_ot = sinkhorn_calibrate(S_cur, iters=sinkhorn_iters, eps=eps)
